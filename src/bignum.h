...
/** C++ wrapper for BIGNUM (OpenSSL bignum) - OpenSSL 1.1+ compatible */
class CBigNum
{
private:
    BIGNUM* bn;
...
public:
    ...
    // Get the raw BIGNUM pointer (for OpenSSL functions)
    ...
    unsigned int GetCompact() const
    {
        ...
        {
            CBigNum tmp;
            BN_rshift(tmp.bn, bn, 8*(nSize-3));
            nCompact = BN_get_word(tmp.bn);
        }
